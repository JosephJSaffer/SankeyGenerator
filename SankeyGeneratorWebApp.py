...
def data(Country,Year):
    ...
    f=open('Balance.csv') #open IEA world balance .csv file downloaded from OECD iLibrary (split into 5 smaller files for easier storage)
    temp=csv.reader(f)
    for row in temp: #scan through all rows for required data
        try:
...
def allocation(IEATES,IEATFC,electricity,heat,Country,Year,Exergy):
    ...
    return sankey(links=links, groups=groups, linkLabelFormat='.0f', linkLabelMinWidth=10, order=order, align_link_types=True)
...
def efficiency(IEATES,IEATFC,electricity,heat,Country,Year,Exergy):
    ...
    return sankey(links=linkseff, groups=groupseff, linkLabelFormat='.0f', linkLabelMinWidth=10, order=ordereff, align_link_types=True)
...
def change(IEATES,IEATFC,electricity,heat,Country,Year,Exergy,Device,Value):
    ...
    return sankey(links=links_changed, groups=groups_changed, linkLabelFormat='.0f', linkLabelMinWidth=10, order=order_changed, align_link_types=True)
